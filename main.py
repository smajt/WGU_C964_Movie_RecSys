...
# settings for needed for all features to work in an executable
...
def insert_viz_3(mark_scores, names):
    ...
    with viz_3_row:
        with ui.matplotlib().figure as fig:
            ...
            ax.plot(index, mark_scores[0], label=names[0])
            ax.legend()
...
with ui.tab_panels(tabs, value='The Recommender System').classes('w-full'):
    ...
    with ui.tab_panel('Visualization 1'):
        with ui.row():
            viz_1_label = ui.label('The model needs to train. Visualizations will populate when ready.')
            viz_1_row = ui.row()
    with ui.tab_panel('Visualization 2'):
        with ui.row():
            viz_2_label = ui.label('The model needs to train. Visualizations will populate when ready.')
            viz_2_row = ui.row()
    with ui.tab_panel('Visualization 3'):
        viz_3_label = ui.label('The model needs to train. Visualizations will populate when ready.')
        viz_3_row = ui.row()
